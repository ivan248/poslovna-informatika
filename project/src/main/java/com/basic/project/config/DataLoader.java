...
import com.basic.project.repository.PreduzeceRepository;
...
//			System.out.println(c.getDatumVazenja());
	...
	private void insertIntoStavkeFakture() {
		...
		sf.setFaktura(fakturaRepository.findById(1L).get());
		...
		sf1.setProizvod(proizvodRepository.getOne(2L));
		sf1.setRabat(10.0);
		sf1.setStopaPDV(5.0);
		sf1.setUkupanIznos(2000.0);
		sf1.setFaktura(fakturaRepository.findById(1L).get());

		stavkaFaktureRepository.save(sf1);

	}

	private void insertIntoPreduzece() {

		Preduzece p = new Preduzece();

		p.setAdresaPreduzeca(adresaRepository.getOne(1L));
		p.setEmail("mojepreduzece@gmail.com");
		p.setKontaktTelefon("011500500");
		p.setNaziv("Preduzece Marinko i sinovi");
		p.setPIB(100200300L);
	...
	private void insertIntoPoslovnaGodina() {
		...
		pg.setPreduzece(preduzeceRepository.getOne(1L));
	...
	private void insertIntoFaktura() {
		 ...
		 * f1.setUkupanIznosBezPDV(1200.6); f1.setUkupanPDV(20.0);
		 * f1.setUkupanRabat(10.0); f1.setUkupnoZaPlacanje(50000.0);
		 * 
		 * List<StavkaFakture> stavkeFakture = new ArrayList<StavkaFakture>();
		 * 
		 * StavkaFakture sf = new StavkaFakture();
		 * 
		 * sf.setIznosPDV(18.0); sf.setJedinicnaCena(2800.0); sf.setKolicina(10);
		 * sf.setOsnovica(15.0); sf.setProizvod(proizvodRepository.getOne(1L));
		 * sf.setRabat(10.0); sf.setStopaPDV(5.0); sf.setUkupanIznos(3000.0);
		 * 
		 * stavkeFakture.add(sf);
		 * 
		 * StavkaFakture sf1 = new StavkaFakture();
		 * 
		 * sf1.setIznosPDV(18.0); sf1.setJedinicnaCena(1800.0); sf1.setKolicina(10);
		 * sf1.setOsnovica(15.0); sf1.setProizvod(proizvodRepository.getOne(1L));
		 * sf1.setRabat(10.0); sf1.setStopaPDV(5.0); sf1.setUkupanIznos(2000.0);
		 * 
	...
	private void insertIntoPoslovniPartner() {
		...
		p1.setEmailPoslovnogPartnera("korporacija@email.com");
		p1.setNazivPoslovnogPartnera("Preduzece 2");
		p1.setPibPoslovnogPartnera(9999);
		p1.setTelefonPoslovnogPartnera(026555000);
		p1.setPoslovniPartnerVrsta(PoslovniPartnerVrsta.PRIVATNO_LICE);

		poslovniPartnerRepository.save(p1);
	...
	private void insertIntoStavkaCenovnika() {
		StavkaCenovnika s = new StavkaCenovnika(1L, 250L, proizvodRepository.findById(1L).get(),
				cenovnikRepository.findById(1L).get());
		StavkaCenovnika s2 = new StavkaCenovnika(2L, 999L, proizvodRepository.findById(3L).get(),
				cenovnikRepository.findById(1L).get());
		StavkaCenovnika s3 = new StavkaCenovnika(3L, 1799L, proizvodRepository.findById(2L).get(),
				cenovnikRepository.findById(1L).get());
